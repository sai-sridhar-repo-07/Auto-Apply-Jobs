...
import { Sparkles, ChevronDown, ChevronUp, BookOpen, Building2 } from "lucide-react";
...
export function InterviewContent() {
  ...
  const [result, setResult] = useState<{ questions: Array<{ question: string; hint: string }>; briefing: string; company_brief: string } | null>(null);
  ...
  return (
    <Tabs defaultValue="new">
      ...
      <TabsContent value="new" className="space-y-4">
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">Generate Interview Prep</CardTitle>
          </CardHeader>
        ...
        {result && (
          <div className="space-y-4">
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm">Round Briefing</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <div className="space-y-3">
              <h3 className="text-sm font-semibold">Interview Questions ({result.questions.length})</h3>
                ...
                <Card key={i}>
                  <CardContent className="py-4">
                    <div className="flex items-start justify-between gap-4 cursor-pointer"
                      ...
                      {expanded === i ? <ChevronUp className="w-4 h-4 text-muted-foreground shrink-0 mt-0.5" />
                        : <ChevronDown className="w-4 h-4 text-muted-foreground shrink-0 mt-0.5" />}
                    </div>
                    {expanded === i && (
                      <div className="mt-3 pt-3 border-t border-border ml-8">
                        <p className="text-xs text-muted-foreground font-medium mb-1">What a strong answer covers:</p>
      ...
      <TabsContent value="guide">
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">STAR Method Guide</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4 text-sm">
            {[
              ...
              <div key={label} className="flex gap-3">
                <span className={`text-xs font-bold px-2 py-1 rounded shrink-0 h-fit ${color}`}>{label}</span>
